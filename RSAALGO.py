...
#We know that: Based on Euclidean theorem: GCD (a, b) = GCD (B, a% B),
#we can find the maximum common factor of a and B
...
def pgcd(a,b):
    if a == 0:
        ...
        x,y,q = pgcd(b%a, a) #As defined above, we are now applying the Euclidean theorem.
        return (x, q - (b//a) * y, y)
       
#Here we check if it is coprime or not and if not, then there is no solution.   
def Mod_Reverse(a,m):
    x,y,q= pgcd(a,m)
    if x!=1:
        return "no solution"
...
#Here we define a function to help us put together the list of prime numbers. 
...
"""
Now we will def the fucntion to encryption and decryption:
keeping in mind that:  msg ^ c ≡ ans (MOD n)
...
msg = int(input("Type your message : "))
RSA_k = RSA_key() 
pub_k = RSA_k['pub']
priv_k = RSA_k['priv']
encrypt = d_encrypt(msg, pub_k[1], pub_k[0])
print("Encrypted messsage is:",encrypt)
